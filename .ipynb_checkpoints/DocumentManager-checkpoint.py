...
#             aDoc.generateReformattedDocument(DocumentManager.outputFolder,DocumentManager.__encoding,cleanContent)
    ...
    def analyseFinalizedContent(self):
        '''
        Return: None
        All words in the cleaned documents will be generated
        at folder output/ALL_Words.txt
        '''
        strContent=" "
        for aDoc in self.__doclist:
        ...
        wordslist.sort()
        wordsDict={'Words':wordslist}
        
        df = pd.DataFrame(wordsDict)
        df_count=df.groupby('Words').size()
        df_count.to_excel(DocumentManager.outputFolder+"ALL_Words.xlsx")  
